...
class Dashboard extends Component {
  ...
  componentDidMount() {
    ...
    API.post(`/dashboard/paquetesXoficinasXfecha_barra`, data).then(
      response => {
  ...
  dataGrafVuelos = () => {
    ...
    API.get("/dashboard/paquetesXvuelosXfecha", {
      params: { inicio: data.inicio, fin: data.fin }
    }).then(response => {
  ...
  dataGrafOficinas = () => {
    ...
    API.post(`/dashboard/paquetesXoficinasXfecha_barra`, data).then(
      response => {
        console.log("Oficinas:", response.data);
        this.setState({ ...this.state, oficinas: response.data }).catch(
          eror => {
            notify.error({
              message: "No se pudo dibujar la grafica"
            });
        ...
        <TheContent>
          <Row>
            <Col span={12} style={colStyle}>
              <Divider>Vuelos más usados</Divider>
              <Row style={colStyle}>
                <Col span={6}>
                  <FormItem label="Rango de fechas" />
                </Col>
                <Col span={10}>
                  <RangePicker
                    locale={locale}
                ...
                <Col span={8}>
                  <Button
                    onClick={this.dataGrafVuelos}
                    disabled={this.state.btnVuelos}
                  >
                    MOSTRAR
                  </Button>
                </Col>
            ...
            <Col span={12} style={colStyle}>
              ...
              <Row style={colStyle}>
                ...
                <Col span={8}>
                  <Button
                    onClick={this.dataGrafOficinas}
                    disabled={this.state.btnPaises}
                  >
                    MOSTRAR
                  </Button>
                </Col>
